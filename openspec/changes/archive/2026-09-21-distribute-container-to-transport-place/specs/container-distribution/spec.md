...
Распределение SHALL быть доступно только при существующих выбранном контейнере, активном ТМ и положительном остатке контейнера. Команда SHALL проверять эти условия до изменения данных и SHALL NOT создавать частичное распределение при нарушении любого условия.

#### Scenario: No active transport place

- **WHEN** выбран контейнер, но активное ТМ отсутствует
