...
ShipmentStore SHALL предоставлять наблюдаемые MobX-данные одного заказа. Каждый экземпляр SHALL владеть независимыми объектами, включая вложенные массивы. Инициализация SHALL работать без сети, browser storage и UI. Новый экземпляр без входных данных SHALL получать исходное демо.

#### Scenario: Independent instances
- **WHEN** создаются два стора и данные первого изменяются в MobX action
- **THEN** второй стор и исходный набор, переданный конструктору, остаются неизменными, включая состав КА

#### Scenario: Fresh demo state
- **WHEN** после изменения одного стора создаётся новый стор без аргументов
- **THEN** новый стор содержит 44 исходные штуки, пустые ТМ и распределение, независимо от предыдущего экземпляра
