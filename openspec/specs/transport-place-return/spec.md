...
`ShipmentStore` SHALL разрешать возврат только при существующем активном ТМ, содержащем хотя бы одну строку распределения с положительным количеством. Команда SHALL проверить условия до изменения данных и SHALL NOT изменять состояние при их нарушении.
...
После успешного возврата активное ТМ SHALL сохраниться и показывать нулевые итоги, `activeTransportPlaceId` SHALL остаться прежним, а `selectedContainerId` SHALL быть сброшен. Контейнеры с положительным восстановленным остатком SHALL снова отображаться слева, а итоги и прогресс SHALL отражать оставшиеся распределения.
...
- **AND** остаток заказа снова равен 44 штукам, прогресс равен 0 и выбор контейнера отсутствует
...
- **THEN** `selectedContainerId` становится `null`
